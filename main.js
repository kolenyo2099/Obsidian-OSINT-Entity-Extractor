    ...
    Readability2.prototype = {
      ...
      _replaceBrs: function(elem) {
        this._forEachNode(this._getAllNodesWithTag(elem, ["br"]), function(br2) {
          ...
          if (replaced) {
            ...
            br2.parentNode.replaceChild(p2, br2);
            next = p2.nextSibling;
            while (next) {
              if (next.tagName == "BR") {
      ...
      **/
      ...
      _unescapeHtmlEntities: function(str2) {
        ...
        return str2.replace(/&(quot|amp|apos|lt|gt);/g, function(_2, tag) {
      ...
      **/
       ...
       * @return void
       **/
      _cleanMatchedNodes: function(e2, filter) {
...
var USER_AGENT = "OSINT-Entity-Extractor/0.1.1 (+https://github.com/thomasjjj/obsidian-osint-ner)";
...
function extractAuthors(byline) {
  if (!byline) return [];
  return byline.split(/,\s*|\s+and\s+|\s*&\s*|;\s*/i).map((a2) => a2.trim()).filter(Boolean);
}
...
function extractLinksAndImages(doc, baseUrl) {
  const links = Array.from(doc.querySelectorAll("a")).map((a2) => {
    try {
      return {
        text: a2.textContent?.trim() ?? "",
        href: new URL(a2.getAttribute("href") || "", baseUrl).href
      };
    } catch {
      return null;
    }
  }).filter((l2) => !!l2 && !!l2.href);
...
function buildPrompt(url, meta, defaultTags, promptTemplate = PROMPT_TEMPLATE) {
  ...
  const tagsArray = Array.isArray(defaultTags) ? defaultTags : defaultTags ? defaultTags.split(",").map((t2) => t2.trim()).filter(Boolean) : [];
  if (tagsArray.length) {
    const tagsBlock = tagsArray.map((t2) => `- ${t2}`).join("\n");
    return `${base}
...
`;
  return vault.create((0, import_obsidian3.normalizePath)(uniquePath), normalizedContent);
}
...
var UrlInputModal = class extends import_obsidian4.Modal {
  ...
  onOpen() {
    ...
    const urlSetting = new import_obsidian4.Setting(contentEl).setName("Article URL");
      ...
      (btn) => btn.setButtonText("Paste").setTooltip("Paste URL from clipboard").onClick(() => {
        void (async () => {
          try {
            ...
            if (!readText) {
              new import_obsidian4.Notice("Clipboard unavailable in this context.");
              return;
            }
            const clip = await readText.call(navigator.clipboard);
            if (!clip) {
              new import_obsidian4.Notice("Clipboard is empty.");
              return;
            }
            ...
            new import_obsidian4.Notice("Couldn't read clipboard in this context.");
          }
        })();
      })
    );
...
var UrlToVaultPlugin = class extends import_obsidian4.Plugin {
  ...
  getApiBaseUrl() {
    if (this.settings.provider === "lmstudio") {
      let url = this.settings.apiBaseUrl?.trim() || "http://127.0.0.1:1234/v1";
  ...
  async runImport(url) {
    ...
    try {
      ...
      new import_obsidian4.Notice("Please enter a valid URL.");
      return;
    }
    const apiKey = await this.getApiKey();
    if (this.settings.provider === "openai" && !apiKey) {
      new import_obsidian4.Notice("Set your OpenAI API key in the plugin settings first.", 6e3);
      return;
    }
    const progress = new import_obsidian4.Notice(renderTwoStepProgress(1, "Fetching article..."), 0);
    try {
      const meta = await fetchAndExtract(normalizedUrl, this.settings.maxChars);
      this.logVerbose("Fetched metadata", meta);
      if (!meta.text) {
        new import_obsidian4.Notice("No article text extracted; sending minimal content to OpenAI.", 6e3);
      }
      ...
      if (this.settings.useCustomPrompt && !promptTemplate) {
        new import_obsidian4.Notice("Custom prompt is empty; using default prompt instead.", 5e3);
      }
      ...
      if (this.settings.includeLinks && meta.links.length) {
        ...
        const linkLines = meta.links.map((l2) => {
          const text = l2.text || l2.href;
          return `- [${text}](${l2.href})`;
        });
      ...
      new import_obsidian4.Notice(`Saved: ${file.path}`);
    } catch (err) {
      console.error(err);
      const message = err instanceof Error ? err.message : String(err);
      new import_obsidian4.Notice(`Import failed: ${message}`, 8e3);
    } finally {
      progress.hide();
    }
  }
