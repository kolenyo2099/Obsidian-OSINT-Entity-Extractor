...
export const OBSIDIAN_FORMATTER_PROMPT = `
...
export function buildPrompt(url: string, meta: ExtractedArticle): string {
  ...
  return OBSIDIAN_FORMATTER_PROMPT.replace("{url}", url)
...
export async function formatWithOpenAI(apiKey: string, model: string, url: string, meta: ExtractedArticle): Promise<string> {
