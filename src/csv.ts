...
export function parseUrlList(text: string): ParsedCsv {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!isValidUrl(url)) {
      errors.push(`Line ${i + 1}: Invalid URL "${line}"`);
      continue;
    }
...
export function parseUrlInput(text: string): ParsedCsv {
  ...
  // Check if it looks like CSV with headers (has commas and multiple columns)
  const firstLine = trimmed.split(/\r?\n/)[0];
  const hasCommas = firstLine.includes(",");
  const looksLikeHeader = /^[a-zA-Z_]/.test(firstLine) && !isValidUrl(firstLine);

  if (hasCommas && looksLikeHeader) {
